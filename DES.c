...
int *translate_bits_To_hex(int *BitSequence, int size);
...
int *translate_hex_To_4bit(char ch)
{
	...
	int digit = 0;
	if ((ch - 48) <= 9) digit = (int)ch - 48;	
	else if (ch == 'a') digit = 10;
...
int main()
{
	...
	else if (choice == 2)
	{
		...
		printf("\n\nGenerating all keys...\n");
		create16Keys();	 // generate all key used in each round
		int *plaintext = decrypt(ciphertext); // encrypt
